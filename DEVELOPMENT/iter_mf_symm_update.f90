...
DOUBLE PRECISION, ALLOCATABLE :: mom(:),exch_field(:),diff(:),weights(:)
...
! Write to output file
  ...
  DO z=1,num_sites,1
    WRITE(11,*) "Moment at site",z,"=",mom(z),"Weight=",weights(z),"Mom/sat_mom=",mom(z)/(g*spin(z))
  END DO
...
! Calculate total moment, uncorrelated spin partition function, Helmholtz and internal energies, and entropy.
 ...
 &    (-1d0*T*1.3806488d-23*dlog(part)/1.602176565d-19))*weights(z)/T
  ...
  END DO
  WRITE(11,*) "Total moment     M=",mom_sum,"mu_B"
